...
const LandingView = ({ onEnter }: { onEnter: () => void }) => (
  <div className="min-h-screen flex flex-col items-center justify-center bg-slate-950 text-slate-100 selection:bg-brand-500 selection:text-white">
    <div 
      onClick={onEnter}
      className="w-full max-w-lg px-4 text-center cursor-pointer group select-none"
    >
      <h1 className="text-2xl font-mono font-bold text-transparent bg-clip-text bg-gradient-to-r from-brand-300 to-brand-500 transition-all duration-300 group-hover:scale-105">
        repository
      </h1>
      <p className="mt-4 text-lg text-slate-400 font-medium tracking-wide group-hover:text-slate-200 transition-colors">
        startup002
      </p>
    </div>
  </div>
    ...
    <div className="min-h-screen bg-slate-950 p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <button className="px-4 py-2 bg-brand-600 hover:bg-brand-500 text-white rounded-lg font-medium transition-colors">
            Add User
          </button>
        </div>
          ...
          <div className="p-4 border-b border-slate-800 flex items-center gap-4">
            <div className="relative flex-grow max-w-md">
              ...
              <input 
                ...
                className="w-full bg-slate-950 border border-slate-800 rounded-lg pl-10 pr-4 py-2 text-slate-200 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500"
              />
            </div>
          </div>
                ...
                <tr key={user.id} className="hover:bg-slate-800/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center">
                      <div className="w-8 h-8 rounded-full bg-brand-900/50 text-brand-400 flex items-center justify-center font-bold text-xs mr-3 border border-brand-800">
                        {user.name.charAt(0)}
                      </div>
                      <div>
...
const AdminDashboardView = ({ onBack }: { onBack: () => void }) => {
  const stats = [
    ...
    { label: 'Bounce Rate', value: '12.23%', change: '-4.5%', trend: 'down' }, // Good down trend
      ...
      <div className="max-w-6xl mx-auto">
            ...
            <div key={i} className="p-6 bg-slate-900 border border-slate-800 rounded-xl hover:border-slate-700 transition-colors">
              ...
              <div className="mt-2 flex items-baseline gap-2">
                ...
                <span className={`text-xs font-medium px-1.5 py-0.5 rounded ${
                  stat.trend === 'up' ? 'text-emerald-400 bg-emerald-400/10' : 'text-emerald-400 bg-emerald-400/10'
                }`}>
                  {stat.change}
                </span>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 h-96">
            ...
            <div className="h-full flex items-center justify-center text-slate-600 border-2 border-dashed border-slate-800 rounded-lg">
              Chart Placeholder
            </div>
          </div>
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 h-96">
              ...
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="flex items-center gap-4 p-3 rounded-lg hover:bg-slate-800/50 transition-colors">
                  ...
                  <div className="flex-grow">
                    <p className="text-sm text-slate-200">New user registration <span className="text-brand-400">@user_{i}</span></p>
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-8">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Visual Bar Chart Construction using CSS Grid/Flex */}
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" defaultValue="Admin" className="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2 text-slate-200 focus:border-brand-500 focus:outline-none" />
                </div>
                <div>
                  <label className="block text-sm text-slate-400 mb-1">Last Name</label>
                  <input type="text" defaultValue="User" className="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2 text-slate-200 focus:border-brand-500 focus:outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-sm text-slate-400 mb-1">Email Address</label>
                <input type="email" defaultValue="admin@startup002.com" className="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2 text-slate-200 focus:border-brand-500 focus:outline-none" />
              </div>
            </div>
          </section>
          ...
          <section className="bg-slate-900 border border-slate-800 rounded-xl p-6">
            ...
            <div className="space-y-4">
              ...
              <button className="flex items-center gap-2 text-sm text-brand-400 hover:text-brand-300 font-medium mt-2">
            ...
            <div className="space-y-3">
              {['Email Alerts', 'Push Notifications', 'Weekly Reports'].map((item) => (
                <label key={item} className="flex items-center gap-3 cursor-pointer group">
                  <div className="w-5 h-5 rounded border border-slate-600 bg-slate-950 flex items-center justify-center group-hover:border-brand-500">
